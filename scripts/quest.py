...
class Quest():
    ...
    def __init__(self,screen, info={}, mouse_pos=(0, 0)):
        self.info = info
    ...
    def update(self):
        ...
        for i in range(4):
            button_pos = (150*i, 400)
            button_size = (100, 100)
            self.screen.blit(pygame.transform.scale(self.assets['button'], button_size), button_pos)
            button_rect = pygame.Rect(button_pos, button_size)
            buttons.append(button_rect)

        for button in buttons:
            if button_rect.colliderect(self.mouse_pos, (10, 10)):
                self.screen.blit(pygame.transform.scale(self.assets['push_button'], button_size), button_pos)